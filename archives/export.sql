...
-- Copiando estrutura para tabela royale.guilds
...
-- Copiando estrutura para tabela royale.guild_members
CREATE TABLE IF NOT EXISTS `guild_members` (
  `guild_tag` varchar(4) NOT NULL,
  `user_id` int(11) NOT NULL,
  `role` enum('OWNER','MANAGER','MEMBER') NOT NULL DEFAULT 'MEMBER',
  PRIMARY KEY (`user_id`,`guild_tag`),
  KEY `FK__guilds` (`guild_tag`),
  CONSTRAINT `FK__guilds` FOREIGN KEY (`guild_tag`) REFERENCES `guilds` (`tag`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
-- Copiando estrutura para tabela royale.hydrus_credits
...
-- Copiando estrutura para tabela royale.hydrus_scheduler
