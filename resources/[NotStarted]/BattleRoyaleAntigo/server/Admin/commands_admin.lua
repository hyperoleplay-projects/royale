...
function gerarCodigo()
    ...
    local tamanho = 10 -- Tamanho do código, sem contar o "duth-"
    ...
    return "DUTH-" .. codigo:upper()
end
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- gerar-codigos
-- -----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("gerar-codigos", function(source, args, rawCmd) 
	local user_id = vRP.getUserId(source)
	...
	for i = 1, tonumber(args[1]) do
		local gerados = gerarCodigo()
		vRP.execute("vRP/addCode", { code = gerados, used = "false" })
		table.insert(codes, gerados)
	end

	local codesString = table.concat(codes, "\n")
	vRP.prompt(source, "Codigos: ", codesString)
end)
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- set-roupa
-- -----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterCommand("set-duthCoins", function(source, args, rawCmd) 
	...
	if source2 then
		Player(source2).state.userData = false
		Wait(500)
		loadUserData(source2, parseInt(args[1]), false)
	end
end)
-- -----------------------------------------------------------------------------------------------------------------------------------------
-- NC
...
RegisterCommand("grupo",function(source,args,rawCmd)
	...
	if source2 then
		Player(source2).state.userData = false
		Wait(500)
		loadUserData(source2, parseInt(args[2]), false)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- Anuncio
...
RegisterCommand("unban",function(source,args,rawCmd)
	...
	local request = exports.ghmattimysql:executeSync("SELECT * FROM bans WHERE user_id = ?",{ parseInt(args[1]) })
    if request[1] then
		exports.ghmattimysql:executeSync("DELETE FROM bans WHERE user_id = ?",{ parseInt(args[1]) })
...
-- whitelist 
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("whitelist", function(source, args, rawCmd) 
	local user_id = vRP.getUserId(source)
...
-- roupas - Command
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("roupas", function(source, args, rawCmd) 
	local user_id = vRP.getUserId(source)
	if not exports["core"]:Group().hasAccessOrHigher(user_id,"dev") then
        return
    end
	TriggerClientEvent("skinshop:openRegisterClotheShop", source)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- rename - Command
-----------------------------------------------------------------------------------------------------------------------------------------
